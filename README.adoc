...
By default only the current AWS region will be configured.
To configure multiple regions, provide a comma separated list of the regions.
...
----
      ...
      - name: Log in to ECR
        ...
        uses: cloudbees-io/configure-ecr-credentials@v1
        with:
          regions: aws-region-1,aws-region-2,aws-region-3
----
